...
if ! pkg-config --atleast-version=1.26 gstreamer-video-1.0; then
  echo "ERROR: GStreamer video 1.26+ is required for native double-click events" >&2
  exit 1
fi
...
gcc "$SCRIPT_DIR/validate_gstreamer_properties.c" \
  -o "$VALIDATOR_DIR/gst-property-validator.exe" \
...
PROPERTY_VALIDATOR_WIN="$(cygpath -w "$VALIDATOR_DIR/gst-property-validator.exe")"
